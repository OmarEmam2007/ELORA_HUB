...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('rps')
        .setDescription('Play Rock Paper Scissors (cinematic).'),

    async execute(interaction) {
        const existing = activeByUser.get(interaction.user.id);
        if (existing && Date.now() - existing < 12_000) {
            return interaction.reply({ content: '⏳ Wait a moment then try again.', ephemeral: true });
        }
        activeByUser.set(interaction.user.id, Date.now());

        const embed = buildStartEmbed(interaction.user.toString());
        const msg = await interaction.reply({ embeds: [embed], components: buildButtons(false), fetchReply: true });
        ...
        const disable = async () => {
            try {
                await interaction.editReply({ components: buildButtons(true) });
            } catch (_) { }
        };
        ...
        collector.on('collect', async (i) => {
            ...
            const resEmbed = buildResultEmbed(interaction.user.toString(), player, bot, result);
            ...
            try {
                ...
                try {
                    await interaction.editReply({ embeds: [resEmbed], files: [file], components: buildButtons(true) });
                } catch (_) { }
            }
        });

        collector.on('end', async (_, reason) => {
            activeByUser.delete(interaction.user.id);
            if (finished) return;
            if (reason === 'time') {
                await disable();
                try {
                    await interaction.followUp({ content: '⌛ Time is up.', ephemeral: true });
                } catch (_) { }
            }
        });
    }
